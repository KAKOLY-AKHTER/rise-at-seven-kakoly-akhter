...
const Hero = () => {
    ...
    const menuDetails = {
        'Services+': {
            width: 'md:w-[1050px]', height: 'h-[320px]',
            items: [
        ...
        'Industries+': {
            width: 'md:w-[570px]', height: 'h-[288px]',
            items: [
                { name: 'B2B Marketing', img: '/3/0B5A68757.webp' },
            ]
        },
        'international+': {
            width: 'md:w-[664px]', height: 'h-[300px]',
            items: [
        ...
        'About+': {
            width: 'md:w-[585px]', height: 'h-[288px]',
            items: [
        ...
        'Blog & Resources+': {
            width: 'md:w-[752px]', height: 'h-[288px]',
            items: [
    ...
    return (
        <section className="w-full px-2 flex items-start justify-center min-h-[100svh]">
            <div className="relative flex flex-col h-[92svh] md:h-screen w-full mx-auto overflow-hidden bg-black font-saans rounded-[22px] shadow-[0_20px_60px_rgba(0,0,0,0.25)]">

                <div className="absolute inset-0 z-0 overflow-hidden">
                    <img
                        src={activeHero.bg}
                        className={`w-full h-full object-cover scale-110 transition-all duration-700 
                        ${isMegaMenu ? 'blur-3xl opacity-50' : 'blur-xl md:blur-md opacity-100'}`}
                        alt="Background"
                    />
                ...
                <header
                    ...
                    className="relative z-[100] flex justify-between items-center px-6 md:px-6 mt-0 md:mt-[-10px] text-white w-full"
                >
                    <div className="font-bold tracking-tighter shrink-0">
                        <svg className="w-[150px] h-[110px] object-contain fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 168 21">
                    ...
                    <nav className="hidden md:flex relative flex-1 justify-center gap-2 text-[16px] font-medium items-center h-[110px]">
                        <div
                            className="absolute bg-white rounded-full transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] pointer-events-none h-[32px] top-1/2 -translate-y-1/2"
                            style={{
                        ...
                        {navItems.map((item) => (
                            <span
                                ...
                                className={`cursor-pointer z-10 transition-colors duration-300 px-[16px] py-[4px] rounded-full relative inline-flex items-center justify-center ${activeMenu === item ? 'text-black' : 'text-white hover:opacity-80'}`}
                            >
                                {item}
                                {item === 'Work' && (
                                    <span className='bg-[#b2f6e3] px-[8px] py-[3px] text-black font-extralight absolute top-[-12px] left-[55px] text-[7px] rounded-[10px]'>25</span>
                                )}
                            </span>
                        ))}

                        {/* Mega Menu Box */}
                        <div className={`absolute top-[90px] left-1/2 -translate-x-1/2 bg-white rounded-[30px] shadow-2xl transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] overflow-visible flex p-[14px] gap-6
                            ${activeMenu && menuDetails[activeMenu]
                                ? `${menuDetails[activeMenu].width} ${menuDetails[activeMenu].height} opacity-100 translate-y-0 pointer-events-auto`
                            ...
                            {activeMenu && menuDetails[activeMenu] && (
                                <>
                                    ...
                                    <div className="flex-1 flex flex-col justify-center items-start px-6">
                                        {activeMenu === 'Services+' && (
                                            <p className="text-gray-400 text-[13px] uppercase tracking-widest mb-4 font-bold leading-none">Core Services</p>
                                        )}
                                        <div className={`grid ${activeMenu === 'Services+' ? 'grid-flow-col grid-rows-4 gap-x-16 gap-y-0' : 'grid-cols-1 gap-y-2'} w-full`}>
                                            {menuDetails[activeMenu].items.map((subItem) => (
                                                <div
                                                    key={subItem.name}
                                                    onMouseEnter={() => setMenuImage(subItem.img)}
                                                    className="relative overflow-hidden group/item cursor-pointer h-[40px] flex items-center justify-start w-fit min-w-[max-content]"
                                                >
                                                    <span className={`block transition-transform duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] group-hover/item:-translate-y-[40px] text-black font-bold leading-[40px] antialiased whitespace-nowrap
                                                        ${isSquareImg ? 'text-[30px]' : 'text-[22px]'}`}>
                                                        {subItem.name}
                                                    </span>
                                                    <span className={`absolute top-full left-0 block transition-transform duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] group-hover/item:-translate-y-[40px] text-black font-bold leading-[40px] antialiased whitespace-nowrap
                                                        ${isSquareImg ? 'text-[30px]' : 'text-[22px]'}`}>
                                                        {subItem.name}
                                                    </span>
                                                </div>
                                    ...
                                    {/* RIGHT: image */}
                                    <div className={`overflow-hidden rounded-[20px] bg-gray-100 transition-all duration-500 relative
    ${isSquareImg ? 'w-[280px] h-full' : 'w-[340px] h-full'}`}>
                                        <img
                                            src={menuImage}
                                        ...
                                        {menuDetails[activeMenu].showViewAll && (
                                            <button className="absolute bottom-3 left-3 hover:rounded-[12px] transition-all duration-500 ease-in-out bg-black text-white w-[200px] py-[10px] px-[20px] rounded-[24px] font-bold group/vbtn overflow-hidden">
                                                <div className="relative h-[18px] overflow-hidden flex flex-col justify-center items-center pointer-events-none">
                                                    <span className="text-[13px] transition-transform duration-300 group-hover/vbtn:-translate-y-6">View All Services ↗</span>
                                                    <span className="absolute top-6 text-[13px] transition-transform duration-300 group-hover/vbtn:-translate-y-6">View All Services ↗</span>
                                                </div>
                                            </button>
                                        )}
                                    </div>
                    ...
                    <div className="flex items-center gap-4 shrink-0">
                        <button className="hidden md:flex relative overflow-hidden bg-white text-black font-bold items-center justify-center h-[45px] w-[160px] rounded-[24px] hover:rounded-[12px] transition-all duration-300 ease-in-out group/btn active:scale-95">
                            <div className="relative h-[16px] w-full flex flex-col items-center overflow-hidden pointer-events-none">
                                <span className="text-[14px] leading-none transition-transform duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] group-hover/btn:-translate-y-7">Get In Touch ↗</span>
                                <span className="absolute top-7 text-[14px] leading-none transition-transform duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] group-hover/btn:-translate-y-7">Get In Touch ↗</span>
                            </div>
                        </button>
                        <div className="md:hidden flex items-center gap-3">
                            ...
                            >
                                <span className="relative w-7 h-7">
                                    <span className={["absolute left-1/2 h-[2px] w-6 -translate-x-1/2 bg-white will-change-transform transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)]", isOpen ? "top-1/2 -translate-y-1/2 rotate-45" : "top-[9px] translate-y-0 rotate-0"].join(" ")} />
                                    <span className={["absolute left-1/2 h-[2px] w-6 -translate-x-1/2 bg-white will-change-transform transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)]", isOpen ? "top-1/2 -translate-y-1/2 -rotate-45" : "top-[17px] translate-y-0 rotate-0"].join(" ")} />
                                </span>
                            </button>
                        </div>
                    </div>
