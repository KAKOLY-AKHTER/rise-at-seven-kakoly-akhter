...
const MobileMenu = ({ isOpen, setIsOpen }) => {
    ...
    const menuData = {
        ...
        'international+': ['US Digital PR', 'Spain Digital PR', 'Germany Digital PR', 'Netherlands Digital PR'],
        'About+': ['About Us', 'Meet The Risers', 'Culture', 'Testimonials']
    };

    return (
        <div
            className={[
                "fixed inset-0 z-[999] p-3 sm:p-4 transition-opacity duration-300",
        ...
        >
            ...
            <div
                className={[
                    "relative h-full w-full rounded-[22px] bg-[#1a1a1a] text-white font-saans overflow-hidden",
                ...
                <div className="h-full w-full flex flex-col p-4 overflow-y-auto">
                    ...
                    <div className="flex justify-between items-center mb-6">
                        <div className="w-[120px]">
                            <svg className="fill-current text-white" viewBox="0 0 168 21" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className="flex-1">
                        {['Services+', 'Industries+', 'international+', 'About+', 'Work', 'Careers', 'Blog', 'Webinar'].map((item) => {
                            const hasSub = menuData[item];
                            const isOpenItem = expandedMenu === item;

                            return (
                                <div key={item} className="mb-2">
                                    <div
                                        onClick={() => hasSub && toggleMenu(item)}
                                        className="flex justify-between items-center py-[6px] cursor-pointer group"
                                    >
                                        ...
                                        {hasSub && (
                                            <div className={`transition-transform duration-300 ${isOpenItem ? 'rotate-180' : 'rotate-0'}`}>
                                    ...
                                    {/* Sub-menus */}
                                    {hasSub && isOpenItem && (
                                        <div className="mt-4 flex flex-col gap-4 pl-1 overflow-hidden transition-all duration-500">
                                            {menuData[item].map((sub) => (
                                                <span key={sub} className="text-[22px] font-bold text-white leading-tight">
                                                    {sub}
                                                </span>
                                            ))}
                                        </div>
                    ...
                    <div className="mt-5 mb-2">
                        <button className="w-full bg-white text-black font-bold py-4 rounded-full text-[17px] flex justify-center items-center gap-2 shadow-lg">
                            Get In Touch ↗
                        </button>
                    </div>
                </div>
